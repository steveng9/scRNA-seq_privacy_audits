...
class Muscat(BaseSingleCellDataGenerator):
    ...
    def generate(self):
        ...
        out_path = f"/tmp/muscat_out.h5ad"
        if not os.path.exists(out_path):
            print(f"Rscript src/generators/models/muscat.r {self.dataset_config['train_count_file']} {self.dataset_config['test_count_file']} {num_test_cells} {out_path}")
            os.system(f"Rscript src/generators/models/muscat.r {self.dataset_config['train_count_file']} {self.dataset_config['test_count_file']} {num_test_cells} {out_path}")
        assert os.path.exists(out_path)

        raw_synth = sc.read_h5ad(out_path)
        synth_cluster_types_dist = Counter(raw_synth.obs['cluster_id'])
        print(synth_cluster_types_dist)
        ...
        cluster_names = {}
        for i, cell_type in enumerate(sorted(map(int, cell_types_dist.keys()))):
            cluster_names[cell_type] = f"cluster{i+1}"
        print(cluster_names)
        ...
        for cell_type in cell_types_dist.keys():
            ...
            print(f"Cell type {cell_type}, num to gen {num_to_gen}")

            cell_type_synth = raw_synth[raw_synth.obs['cluster_id'] == cluster_names[cell_type]]
            assert raw_synth.n_obs >= num_to_gen
            cell_type_synth = cell_type_synth[:num_to_gen]
            ...
            for i, row_idx in enumerate(cell_indices):
                ...
                #print(cell_type_synth[i])
                #try:
                #    synthetic_counts[row_idx, self.hvg_mask] = np.expm1(cell_type_synth[i].X.toarray().astype(float).flatten())
                #except:
                #    synthetic_counts[row_idx, self.hvg_mask] = np.expm1(cell_type_synth[i-1].X.toarray().astype(float).flatten())
            ...
            #synthetic_counts[cell_indices][:, self.hvg_mask] = np.transpose(syn)
