"""
train.py — train "lowrank" scDesign2 copulas.

Identical to experiments/dp/v2/train.py except it calls
src/sdg/scdesign2/scdesign2_lowrank.r instead of scdesign2_v2.r. The only
difference in that R script is that it additionally saves the per-cell
normal-quantile matrix (quantile_normal) into each cell type's .rds -- needed
by src/sdg/dp/lowrank.py's mechanism (see that module's docstring for why).
cov_mat's computation is otherwise byte-for-byte identical to v2, so this is
a genuine re-fit (not reusable from v2_no_dp's .rds files, which lack
quantile_normal), but statistically the same model.

Saves to:
    ~/data/scMAMAMIA/{dataset}/scdesign2/lowrank_no_dp/{nd}d/{trial}/models/

This directory is the single shared source for ALL (rank, epsilon)
combinations generate.py produces -- training does not depend on rank or
epsilon, only the noise-injection step does.
"""
...
R_SCRIPT  = os.path.join(REPO_ROOT, "src", "sdg", "scdesign2", "scdesign2_lowrank.r")
...
def train_one(dataset, nd, trial, max_workers=DEFAULT_MAX_WORKERS, force=False):
    ...
    out_root   = os.path.join(base, "scdesign2", "lowrank_no_dp", f"{nd}d", str(trial))
    ...
    print(f"[lowrank/train] {dataset} {nd}d t{trial} — loading full anndata (backed)…", flush=True)
    ...
    print(f"[lowrank/train]   train donors={len(train_donors)}, train cells={train_mask.sum()}", flush=True)
    ...
    print(f"[lowrank/train]   {hvg_mask.sum()} HVGs", flush=True)
    ...
    for ct in unique_cts:
        ...
        if os.path.exists(out_rds) and not force:
            print(f"[lowrank/train]   skip {ct} — {out_rds} exists", flush=True)
            continue
    ...
    if not cell_types_to_train:
        print(f"[lowrank/train]   all cell types already trained.", flush=True)
        _write_provenance(out_root, dataset, nd, trial, full_h5ad, hvg_path, splits_dir,
                          n_cell_types=len(unique_cts))
        return out_root

    with tempfile.TemporaryDirectory(prefix="lowrank_train_") as tmp_dir:
        hvg_h5ad = os.path.join(tmp_dir, "hvg_train.h5ad")
        X_hvg.write(hvg_h5ad)
        del X_hvg

        ctx = multiprocessing.get_context("spawn")
        print(f"[lowrank/train]   launching parallel R training "
        ...
        with ProcessPoolExecutor(max_workers=max_workers, mp_context=ctx) as exe:
            ...
            for fut in as_completed(futures):
                ...
                if status == "ok":
                    print(f"[lowrank/train]   ✓ {ct}", flush=True)
                else:
                    print(f"[lowrank/train]   ✗ {ct} — {err[:200] if err else ''}", flush=True)

    _write_provenance(out_root, dataset, nd, trial, full_h5ad, hvg_path, splits_dir,
                      n_cell_types=len(unique_cts))
    print(f"[lowrank/train] done: {out_root}", flush=True)
    return out_root
